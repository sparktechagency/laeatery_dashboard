...
const UpdateTermsCondition = ({ description: initialDescription }: TProps) => {
  const [description, setDescription] = useState(initialDescription);
  const editor = useRef(null);
  const [createUpdateTermsCondition, { isLoading }] =
    ...
    <>
      <div className="h-full p-4">
        <div className="bg-[#F6F6F6] h-[90%] p-4">
           <h1 className="m-6 font-bold text-2xl text-center">Update Terms-Conditions</h1>
          <JoditEditor
            ref={editor}
            value={description}
